...
import { getSubprocesses, createSubprocess, deleteSubprocess } from "../../services/subprocessService";
...
const SubprocessManagement = () => {
  ...
  const handleCreateSubprocess = async () => {
    ...
    try {
      const createdSubprocess = await createSubprocess(newSubprocess, selectedProcessId);
      setSubprocesses([...subprocesses, createdSubprocess]);
      setNewSubprocess("");
    } catch (error) {
  ...
  return (
    <div className="min-h-screen p-6 bg-gray-900 text-white">
      ...
      <div className="flex space-x-2 mb-4">
        ...
        <button onClick={handleCreateSubprocess} className="bg-green-600 px-4 py-2 rounded-lg">
          Adicionar
        </button>
      </div>
      ...
      <ul>
        {subprocesses.map((sub) => (
          <li key={sub.id} className="flex justify-between bg-white text-black p-2 rounded-lg mb-2">
            {sub.name}
            <button
